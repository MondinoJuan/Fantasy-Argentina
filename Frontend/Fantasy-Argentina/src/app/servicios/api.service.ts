...
export interface TranslatePricesJob {
  jobId: string;
  status: BackgroundJobStatus;
  leagueId: number;
  createdAt: string;
...
export class ApiService {
  ...
  postRealPlayerTranslatePricesByLeague(payload: { leagueId: number }) {
    return this.http.post<{ message: string; data: TranslatePricesJob }>(`${this.url}/real-players/translate-prices-by-league`, payload);
  }

  getTranslatePricesByLeagueJob(jobId: string) {
    return this.http.get<{ message: string; data: TranslatePricesJob }>(
